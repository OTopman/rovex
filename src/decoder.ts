 ...
 */
function parseHeader(buf: Buffer, offset: number, requireSrid: boolean): number {
  ...
  if (requireSrid) {
    if (!hasSrid) return -1;
    if (buf.length < offset + 9) return -1;
    const srid = readUint32(buf, offset + OFFSET_SRID, littleEndian);
    if (srid !== SRID_WGS84) return -1;
    headerLength = 9;
  } else if (hasSrid) {
    if (buf.length < offset + 9) return -1;
    const srid = readUint32(buf, offset + OFFSET_SRID, littleEndian);
...
export function decodePoint(buf: Buffer, offset = 0, requireSrid = true): Coordinate | null {
  const header = parseHeader(buf, offset, requireSrid);
  if (header === -1) return null;

  const geometryType = header >> 11;
  if (geometryType !== WKB_TYPE_POINT) return null;
...
export function decodeLineString(buf: Buffer, offset = 0, requireSrid = true): Coordinate[] {
  const header = parseHeader(buf, offset, requireSrid);
  if (header === -1) return [];

  const geometryType = header >> 11;
  if (geometryType !== WKB_TYPE_LINESTRING) return [];
  ...
  if (buf.length < startOffset + 4) return [];
  ...
  if (buf.length < requiredBytes) return [];

  const result = new Array<Coordinate>(numPoints);
...
export function decodePolygon(buf: Buffer, offset = 0, requireSrid = true): Coordinate[][] {
  const header = parseHeader(buf, offset, requireSrid);
  if (header === -1) return [];

  const geometryType = header >> 11;
  if (geometryType !== WKB_TYPE_POLYGON) return [];
  ...
  const result = new Array<Coordinate[]>(numRings);
  for (let r = 0; r < numRings; r++) {
    if (buf.length < currentOffset + 4) return [];
    ...
    const ring = new Array<Coordinate>(numPoints);
...
export function decodeMultiPoint(buf: Buffer, offset = 0, requireSrid = true): Coordinate[] {
  const header = parseHeader(buf, offset, requireSrid);
  if (header === -1) return [];

  const geometryType = header >> 11;
  if (geometryType !== WKB_TYPE_MULTIPOINT) return [];
  ...
  const result = new Array<Coordinate>(numGeoms);
  for (let i = 0; i < numGeoms; i++) {
    const subHeader = parseHeader(buf, currentOffset, false);
    if (subHeader === -1) return [];

    const subType = subHeader >> 11;
    if (subType !== WKB_TYPE_POINT) return [];
...
export function decodeMultiLineString(buf: Buffer, offset = 0, requireSrid = true): Coordinate[][] {
  const header = parseHeader(buf, offset, requireSrid);
  if (header === -1) return [];

  const geometryType = header >> 11;
  if (geometryType !== WKB_TYPE_MULTILINESTRING) return [];
  ...
  for (let i = 0; i < numGeoms; i++) {
    ...
    const subType = subHeader >> 11;
    if (subType !== WKB_TYPE_LINESTRING) return [];
    ...
    if (buf.length < startSub + 4) return [];
    ...
    const line = new Array<Coordinate>(numPoints);
...
export function decodeMultiPolygon(buf: Buffer, offset = 0, requireSrid = true): Coordinate[][][] {
  const header = parseHeader(buf, offset, requireSrid);
  if (header === -1) return [];

  const geometryType = header >> 11;
  if (geometryType !== WKB_TYPE_MULTIPOLYGON) return [];
  ...
  for (let i = 0; i < numGeoms; i++) {
    ...
    const subType = subHeader >> 11;
    if (subType !== WKB_TYPE_POLYGON) return [];
    ...
    const stride = 16 + (subHasZ ? 8 : 0) + (subHasM ? 8 : 0);
    const polygon = new Array<Coordinate[]>(numRings);
