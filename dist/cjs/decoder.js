...
exports.decodeLineString = decodeLineString;
exports.decodePolygon = decodePolygon;
exports.decodeMultiPoint = decodeMultiPoint;
exports.decodeMultiLineString = decodeMultiLineString;
...
function parseHeader(buf, offset, requireSrid) {
    ...
    const rawType = (0, reader_js_1.readUint32)(buf, offset + constants_js_1.OFFSET_WKB_TYPE, littleEndian);
    const hasSrid = (rawType & constants_js_1.EWKB_SRID_FLAG) !== 0;
    const hasZ = (rawType & constants_js_1.EWKB_Z_FLAG) !== 0;
    const hasM = (rawType & constants_js_1.EWKB_M_FLAG) !== 0;
    ...
    if (requireSrid) {
        ...
        if (buf.length < offset + 9)
            return -1;
        const srid = (0, reader_js_1.readUint32)(buf, offset + constants_js_1.OFFSET_SRID, littleEndian);
        if (srid !== constants_js_1.SRID_WGS84)
            return -1;
        headerLength = 9;
    }
    else if (hasSrid) {
...
function readCoordinate(buf, offset, littleEndian, hasZ, hasM) {
    const x = (0, reader_js_1.readFloat64)(buf, offset, littleEndian);
    const y = (0, reader_js_1.readFloat64)(buf, offset + 8, littleEndian);
    if (hasZ && hasM) {
        const z = (0, reader_js_1.readFloat64)(buf, offset + 16, littleEndian);
        const m = (0, reader_js_1.readFloat64)(buf, offset + 24, littleEndian);
        ...
        const m = (0, reader_js_1.readFloat64)(buf, offset + 16, littleEndian);
...
// Point Decoder
// ---------------------------------------------------------------------------
function decodePoint(buf, offset = 0, requireSrid = true) {
    const header = parseHeader(buf, offset, requireSrid);
    if (header === -1)
        return null;
    const geometryType = header >> 11;
    if (geometryType !== constants_js_1.WKB_TYPE_POINT)
        return null;
...
// LineString Decoder
// ---------------------------------------------------------------------------
function decodeLineString(buf, offset = 0, requireSrid = true) {
    const header = parseHeader(buf, offset, requireSrid);
    if (header === -1)
        return [];
    const geometryType = header >> 11;
    if (geometryType !== constants_js_1.WKB_TYPE_LINESTRING)
        return [];
    const littleEndian = (header & 1) === 1;
    const hasZ = (header & 2) === 2;
    const hasM = (header & 4) === 4;
    ...
    if (buf.length < startOffset + 4)
        return [];
    const numPoints = (0, reader_js_1.readUint32)(buf, startOffset, littleEndian);
    if (numPoints === 0)
        return [];
    const stride = 16 + (hasZ ? 8 : 0) + (hasM ? 8 : 0);
...
// Polygon Decoder
// ---------------------------------------------------------------------------
function decodePolygon(buf, offset = 0, requireSrid = true) {
    const header = parseHeader(buf, offset, requireSrid);
    if (header === -1)
        return [];
    const geometryType = header >> 11;
    if (geometryType !== constants_js_1.WKB_TYPE_POLYGON)
        return [];
    const littleEndian = (header & 1) === 1;
    const hasZ = (header & 2) === 2;
    const hasM = (header & 4) === 4;
    ...
    if (buf.length < startOffset + 4)
        return [];
    const numRings = (0, reader_js_1.readUint32)(buf, startOffset, littleEndian);
    if (numRings === 0)
        return [];
    ...
    for (let r = 0; r < numRings; r++) {
        if (buf.length < currentOffset + 4)
            return [];
        const numPoints = (0, reader_js_1.readUint32)(buf, currentOffset, littleEndian);
        currentOffset += 4;
...
// MultiPoint Decoder
// ---------------------------------------------------------------------------
function decodeMultiPoint(buf, offset = 0, requireSrid = true) {
    const header = parseHeader(buf, offset, requireSrid);
    if (header === -1)
        return [];
    const geometryType = header >> 11;
    if (geometryType !== constants_js_1.WKB_TYPE_MULTIPOINT)
        return [];
    const littleEndian = (header & 1) === 1;
    const headerLength = (header >> 3) & 0xff;
    const startOffset = offset + headerLength;
    if (buf.length < startOffset + 4)
        return [];
    const numGeoms = (0, reader_js_1.readUint32)(buf, startOffset, littleEndian);
    let currentOffset = startOffset + 4;
    ...
    for (let i = 0; i < numGeoms; i++) {
        ...
        if (subType !== constants_js_1.WKB_TYPE_POINT)
            return [];
...
// ---------------------------------------------------------------------------
function decodeMultiLineString(buf, offset = 0, requireSrid = true) {
    const header = parseHeader(buf, offset, requireSrid);
    if (header === -1)
        return [];
    const geometryType = header >> 11;
    if (geometryType !== constants_js_1.WKB_TYPE_MULTILINESTRING)
        return [];
    const littleEndian = (header & 1) === 1;
    const headerLength = (header >> 3) & 0xff;
    const startOffset = offset + headerLength;
    ...
    for (let i = 0; i < numGeoms; i++) {
        ...
        if (subHeader === -1)
            return [];
        const subType = subHeader >> 11;
        if (subType !== constants_js_1.WKB_TYPE_LINESTRING)
            return [];
        const subLE = (subHeader & 1) === 1;
        const subHasZ = (subHeader & 2) === 2;
        const subHasM = (subHeader & 4) === 4;
        ...
        if (buf.length < startSub + 4)
            return [];
        const numPoints = (0, reader_js_1.readUint32)(buf, startSub, subLE);
        const stride = 16 + (subHasZ ? 8 : 0) + (subHasM ? 8 : 0);
...
// ---------------------------------------------------------------------------
function decodeMultiPolygon(buf, offset = 0, requireSrid = true) {
    const header = parseHeader(buf, offset, requireSrid);
    if (header === -1)
        return [];
    const geometryType = header >> 11;
    if (geometryType !== constants_js_1.WKB_TYPE_MULTIPOLYGON)
        return [];
    const littleEndian = (header & 1) === 1;
    const headerLength = (header >> 3) & 0xff;
    const startOffset = offset + headerLength;
    ...
    for (let i = 0; i < numGeoms; i++) {
        ...
        if (subHeader === -1)
            return [];
        const subType = subHeader >> 11;
        if (subType !== constants_js_1.WKB_TYPE_POLYGON)
            return [];
        const subLE = (subHeader & 1) === 1;
        const subHasZ = (subHeader & 2) === 2;
        const subHasM = (subHeader & 4) === 4;
        ...
        if (buf.length < startSub + 4)
            return [];
        const numRings = (0, reader_js_1.readUint32)(buf, startSub, subLE);
        ...
        for (let r = 0; r < numRings; r++) {
            if (buf.length < currentOffset + 4)
                return [];
            const numPoints = (0, reader_js_1.readUint32)(buf, currentOffset, subLE);
            currentOffset += 4;
            const requiredBytes = currentOffset + numPoints * stride;
            if (buf.length < requiredBytes)
                return [];
